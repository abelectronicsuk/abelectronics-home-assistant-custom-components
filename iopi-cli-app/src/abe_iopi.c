...
void set_interrupt_type_on_pin(char address, char pin, char value) {
	...
	*/
	...
	char cval = 0;
	if (pin < 8) {
		cval = read_byte_data(address, INTCONA);
		cval = updatebyte(cval, pin, value);
		write_byte_data(address, INTCONA, cval);
	} else {
		cval = read_byte_data(address, INTCONB);
		cval = updatebyte(cval, pin - 8, value);
		write_byte_data(address, INTCONB, cval);
	}
}
...
void set_interrupt_defaults_on_pin(char address, char pin, char value) {
	...
	*/
	...
	char cval = 0;
	if (pin < 8) {
		cval = read_byte_data(address, DEFVALA);
		cval = updatebyte(cval, pin, value);
		write_byte_data(address, DEFVALA, cval);
	} else {
		cval = read_byte_data(address, DEFVALB);
		cval = updatebyte(cval, pin - 8, value);
		write_byte_data(address, DEFVALB, cval);
	}
}
...
char read_interrupt_status_on_pin(char address, char pin) {
	...
	*/
	pin = pin - 1;
	if (pin < 8) {
		return (checkbit(read_byte_data(address, INTFA), pin));
	} else {
		pin = pin - 8;
		return (checkbit(read_byte_data(address, INTFB), pin));
	}
}
...
char read_interrupt_capture_on_pin(char address, char pin) {
	...
	*/
	pin = pin - 1;
	if (pin < 8) {
		return (checkbit(read_byte_data(address, INTCAPA), pin));
	} else {
		pin = pin - 8;
		return (checkbit(read_byte_data(address, INTCAPB), pin));
	}
}
