...
class abelectronicsiopiSwitch(ToggleEntity):
    ...
    def __init__(self, pinname, pin, invert_logic, bus):
        ...
        # read initial state from hardware (blocking) on executor
        self._state = False
        try:
            ...
            if pindir == 1:
                self.iobus.set_pin_direction(self.targetpin, 0)
            self._state = bool(self.iobus.read_pin(self.targetpin))
            if invert_logic:
                self.iobus.invert_pin(self.targetpin, 1)
        except Exception as e:
            _LOGGER.exception("IOPi init error for pin %s: %s", self.targetpin, e)
    ...
    async def async_update(self):
        val = await self.hass.async_add_executor_job(self.iobus.read_pin, self.targetpin)
        self._state = bool(val)

    async def async_turn_on(self, **kwargs):
        """Turn the device on (runs hardware write in executor)."""
        try:
            await self.hass.async_add_executor_job(self.iobus.write_pin, self.targetpin, 1)
            self._state = True
        except Exception:
            _LOGGER.exception("Failed to turn on pin %s", self.targetpin)
        # notify HA of state change
        self.async_write_ha_state()

    async def async_turn_off(self, **kwargs):
        """Turn the device off (runs hardware write in executor)."""
        try:
            await self.hass.async_add_executor_job(self.iobus.write_pin, self.targetpin, 0)
            self._state = False
        except Exception:
            _LOGGER.exception("Failed to turn off pin %s", self.targetpin)
        self.async_write_ha_state()